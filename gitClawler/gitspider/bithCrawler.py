...
@function： https://github.com/BithumbCoin
"""
import datetime
...
class crawler:
    ...
    def __del__(self):
        ...
        self.wb.save("./Excel/" + self.day_time.strftime('%Y-%m-%d') + "_" + end_time.strftime('%m-%d')+ "_bithumb" + ".xlsx")
        self.driver.quit()
        print("GitHub_Crawler is ending...")

    def start(self):
        print("正在抓取 " + str(self.day_time.day) + "日 的提交记录")
        self.driver.get("https://github.com/BithumbCoin")
        times = self.driver.find_elements_by_xpath('//*[@id="org-repositories"]/div[1]/div/li/div[3]/relative-time')
    ...
    def get_commit(self, project_name, branch):
        flag = False
        commits = []
        url = "https://github.com/BithumbCoin/" + project_name + "/commits/" + branch
        self.driver.get(url)
        while True:
    ...
    def get_branchs(self, project_name):
        self.driver.get("https://github.com/BithumbCoin/" + project_name + "/branches")
        x = self.driver.find_elements_by_xpath(
            '//*[@id="branch-autoload-container"]/div/div[2]/div/span[1]/a')
        res = []
